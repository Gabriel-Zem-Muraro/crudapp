...
import MostraDados from "@/componentes/mostraDados";
import FormularioConsultatodos from "@/formularios/formularioConsultaTodos";
...
export default function Home() {
  //CONSULTAR ID
  const [
    setMostrarFormularioConsultaEBotaoConsultaId,
    setMostrarFormularioConsultaId,
  ] = useState(false); //Mostra o formulario para digitar o ID

  const [dadosUsuario, setDadosUsuario] = useState<Usuario | null>(null); //Armazena os dados do usuário retornados pela api na variavel dadosUsuario
  const [erro, setErro] = useState(""); //Armazena a mensagem de erro tratada na api se falhar
  const [mostrarDados, setMostrarDados] = useState(false);

  //CONSULTA ATIVOS / INATIVOS
  const [
    setMostraFormularioConsultaTodosEBotaoConsultaAtivo,
    setMostrarFormularioConsultaTodos,
  ] = useState(false); //Mostra o formulario para selecionar o ativo ou inativo

  //CRIAR
  const [mostrarFormularioEBotaoCria, setMostrarFormularioCria] =
    useState(false);

  //DELETAR
  const [mostrarFormularioEBotaoDeletar, setMostrarFormularioDeleta] =
    useState(false);

  //ATUALIZAR PARCIAL
  const [
    mostrarFormularioEBotaoAtualizaParcial,
    setMostrarFormularioAtualizaP,
  ] = useState(false);

  //ATUALIZAR TOTAL
  const [mostrarFormularioEBotaoAtualizaTotal, setMostrarFormularioAtualizaT] =
    useState(false);
  ...
  const consultarUsuario = async (id: string) => {
    //Recebe o id como contexto
  ...
  const handleConsultaTodos = () => {
    setMostrarFormularioConsultaTodos(true); //Faz abrir o formulario na tela
    setDadosUsuario(null); //Limpa os dados enviados pela api anteriormente
    setErro(""); //Limpa a mensagem de erro também
  };
  const handleVoltaConsultaTodos = () => {
    setMostrarFormularioConsultaTodos(false);
  };

  //FUNÇAO PARA CONSULTAR OS USUARIOS ATIVOS OU NÃO ATIVOS
  const consultaAtivosEInativos = async (isActive: boolean) => {
    //Recebe true ou false como contexto
    setErro(""); //Limpa os erros anteriores
    try {
      const usuarios = await userService.getAll(isActive);
      setDadosUsuario(usuarios);
      console.log(usuarios);
    } catch (error: any) {
      console.error("Erro ao consultar usuarios");
      setErro(error.response?.data?.message || "Erro ao consultar usuário");
      setDadosUsuario(null);
    }
  };
  ...
  //MUDA O ESTADO DO FORMULARIO DELETAR
  ...
  return (
    <div className="justify-items-center">
      <main className="flex flex-col gap-[32px] row-start-2 justify-items-center sm:items-start px-10 py-10 outline outline-offset-2 outline-stone-500 my-8">
        ...
        <section className="flex items-center bg-zinc-800 p-3 rounded-4xl outline-2 outline-offset-2 outline-stone-500 w-[100%]">
          <div className="flex flex-col items-center w-full outline-offset-2 outline-stone-500">
            {mostrarDados && dadosUsuario && (
              <>
                <MostraDados label="ID" value={dadosUsuario.id} />
                <MostraDados label="Nome" value={dadosUsuario.name} />
                <MostraDados label="Email" value={dadosUsuario.email} />
                <MostraDados label="Idade" value={dadosUsuario.idade} />
              </>
            )}
          </div>
        </section>
        ...
        {setMostrarFormularioConsultaEBotaoConsultaId && (
          <FormularioConsulta onConsultar={consultarUsuario} />
        )}
        {/*MOSTRAR FORMULARIO PARA CONSULTAR TODOS*/}
        {setMostraFormularioConsultaTodosEBotaoConsultaAtivo && (
          <FormularioConsultatodos onConsultarAtivos={consultaAtivosEInativos}/>
        )}
        ...
        <div className="flex flex-col items-center w-full outline-offset-2 outline-stone-500">
          ...
          {setMostrarFormularioConsultaEBotaoConsultaId && (
            <Botao
              name="Voltar consulta"
              value="voltar"
              onClick={handleVoltaConsulta}
            />
          )}
          {/*ESCONDE FORMULARIO PARA CONSULTAR*/}
          {mostrarFormularioEBotaoCria && (
            <Botao
              name="Voltar Criar"
              value="voltar"
              onClick={handleVoltarCria}
            />
          )}
          {/*ESCONDE FORMULARIO PARA DELETAR*/}
          {mostrarFormularioEBotaoDeletar && (
            <Botao
              name="Voltar Deletar"
              value="voltar"
              onClick={handleVoltaDeleta}
            />
          )}
          {/*ESCONDE FORMULARIO PARA ATUALIZAR PARCIAL*/}
          {mostrarFormularioEBotaoAtualizaParcial && (
            <Botao
              name="Voltar Atualizar Parcial"
              value="voltar"
              onClick={handleVoltaAtualizaP}
            />
          )}
          {/*ESCONDE FORMULARIO PARA ATUALIZAR TOTAL*/}
          {mostrarFormularioEBotaoAtualizaTotal && (
            <Botao
              name="Voltar Atualizar Total"
              value="voltar"
              onClick={handleVoltaAtualizaT}
            />
          )}
        </div>
      </main>

...
export default function FormularioAtualizaParcial () {
  ...
  return(
      <form onSubmit={aoSalvar} className="flex flex-col justify-center items-center bg-zinc-800 p-3 rounded-4xl h-full w-full outline outline-offset-2 outline-stone-500">
        ...
        <CamposDeTexto label="Nome" placeholder="Digite seu nome" value={nome} aoAlterado={(valor: string) => setNome(valor)} />
        <CamposDeTexto label="Idade" placeholder="Digite sua idade" value={idade} aoAlterado={(valor: string) => setIdade(valor)}/>
        <CamposDeTexto label="E-mail" placeholder="Digite seu e-mail" value={email} aoAlterado={(valor: string) => setEmail(valor)}/>
        <CamposDeTexto label="Senha" placeholder="Digite sua senha" value={senha} aoAlterado={(valor: string) => setSenha(valor)}/>
